...
/**
 * Thrown when a query is supposed to affect rows, but don't
 * @extends BaseError
 * @constructor
 */
error.NoAffectedRowsError = function(message, errors) {
  error.BaseError.apply(this, arguments);
  this.name = 'SequelizeNoAffectedRowsError';
  this.message = message;
};
util.inherits(error.NoAffectedRowsError, error.BaseError);
